...
if __name__ == "__main__":
    ...
    r = df.iloc[i]
    image = imread(os.path.join('data/market-train/train', r.name))
    pose_cords = load_pose_cords_from_strings(r.keypoints_y, r.keypoints_x)
    pose_mask = cords_to_map(pose_cords, image.shape[:2])
